...
async fn rebuild_populates_correct_field_values_after_successful_run() {
    ...
    // Verify field values in current_node_state after rebuild
    let row = sqlx::query(
...
async fn rebuild_preserves_prior_success_state_after_failed_run() {
    ...
    let row = sqlx::query(
        "SELECT checksum, relation_database, relation_schema, last_success_at \
         FROM current_node_state WHERE unique_id = 'model.demo.orders'"
    ).fetch_one(&pool).await.unwrap();
    use sqlx::Row;
    let success_checksum: Option<String> = row.get("checksum");
    let _success_relation_db: Option<String> = row.get("relation_database");
    let success_at: Option<chrono::DateTime<chrono::Utc>> = row.get("last_success_at");
    assert_eq!(success_checksum.as_deref(), Some("good_checksum"));
    ...
    // Verify: status should reflect the latest (failed) run, but promoted fields
    // (checksum, relation, last_success_at) should be preserved from the successful run
    let row = sqlx::query(
        "SELECT status, checksum, relation_database, relation_schema, last_success_at \
         FROM current_node_state WHERE unique_id = 'model.demo.orders'"
    ).fetch_one(&pool).await.unwrap();
...
async fn rebuild_accumulates_state_across_multiple_runs_with_different_nodes() {
    ...
    // Verify both nodes exist in current_node_state
