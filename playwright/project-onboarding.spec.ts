...
async function createProject(page, repoUrl) {
  ...
  await expect(page.getByText('Reach repository')).toBeVisible();
  await expect(page.getByText('Validate project path')).toBeVisible();
  await expect(page.getByText('Resolve default branch')).toBeVisible();
  await expect(page.getByText('Read dbt project metadata')).toBeVisible();
...
test.describe('project onboarding', () => {
  ...
  test('shows validation failure for an invalid project path and lets the user retry', async ({ page, app }) => {
    ...
    await expect(page.getByText('Validation Steps')).toBeVisible();
    await expect(page.locator('#project-draft-panel').getByText('failed', { exact: true })).toHaveCount(2);
  ...
  test('creates a remote environment from the project card', async ({ page, app }) => {
    ...
    await expect(modalForm.locator('select[name="git_branch"]').last()).toHaveValue('main');
    await modalForm.locator('input[name="slug"]').fill('dev');
    ...
    await expect(page).toHaveURL(
      new RegExp(`${app.baseUrl.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}/ui/projects/.+/environments/dev$`),
    );
    await expect(page.getByRole('heading', { name: /jaffle_shop_project \/ dev/ })).toBeVisible();
